...
router.route('/').get(protect, authorize('superuser', 'manager'), validatePagination, getUsers).post(protect, authorize('superuser', 'manager'), createModifyLimiter, sanitizeBody, validateUser, createUser);
...
router.route('/:id')
    ...
    .put(protect, authorize('superuser', 'manager'), createModifyLimiter, validateObjectId('id'), sanitizeBody, validateUser, updateUser)
