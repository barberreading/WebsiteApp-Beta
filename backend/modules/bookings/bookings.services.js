...
const getBookings = async (user) => {
  ...
  if (user.role === 'staff') {
    ...
    // Superusers see all bookings including test user bookings
...
const getWeeklyBookings = async (startDate, endDate, user) => {
  if (!startDate || !endDate) {
    throw new ErrorResponse('Start date and end date are required', 400);
  }
  ...
  // For non-superusers, exclude test user bookings
...
const getBookedStaffForClient = async (clientId, user) => {
  // Ensure the user can only access their own bookings
  if (user.role === 'client' && user.id !== clientId) {
    throw new ErrorResponse('Not authorized to access this data', 401);
  }

  const bookings = await Booking.find({ client: clientId })
    .populate('staff', 'firstName lastName profilePicture role')
    .select('staff')
    .lean();

  // Get unique staff members who have bookings with this client
  const staffMap = new Map();
  bookings.forEach(booking => {
    if (booking.staff && !staffMap.has(booking.staff._id.toString())) {
      staffMap.set(booking.staff._id.toString(), booking.staff);
    }
  });
...
const getBookingsForClientAndStaff = async (clientId, staffId, user) => {
  // Ensure the user can only access their own bookings
  if (user.role === 'client' && user.id !== clientId) {
    throw new ErrorResponse('Not authorized to access this data', 401);
  }
