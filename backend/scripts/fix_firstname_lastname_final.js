...
async function fixFirstNameLastNameFinal() {
  try {
    logger.log('Fixing firstName and lastName fields...');
    ...
    for (const user of users) {
      if (user.name && (!user.firstName || !user.lastName)) {
        ...
        logger.log(`Updating ${user.name} -> firstName: '${firstName}', lastName: '${lastName}'`);
    ...
    updatedUsers.forEach(user => {
      logger.log(`- Name: ${user.name} | FirstName: ${user.firstName} | LastName: ${user.lastName} | Email: ${user.email}`);
    });
