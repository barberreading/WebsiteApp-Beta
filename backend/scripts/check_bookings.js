...
async function checkBookings() {
  try {
    ...
    logger.log('Total bookings:', count);
    ...
    logger.log('Bookings in problematic date range:', rangeCount);
    ...
    logger.log('Sample bookings (raw):', JSON.stringify(sampleBookings, null, 2));
    logger.log('Sample response size (bytes):', JSON.stringify(sampleBookings).length);
    
    // Check if there are too many bookings in the range
    if (rangeCount > 100) {
      logger.log('WARNING: Large number of bookings in date range may cause performance issues');
    }
