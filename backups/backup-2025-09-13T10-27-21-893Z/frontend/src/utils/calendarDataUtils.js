...
export const fetchBookingAlertsForUser = async (currentUser, hasRole) => {
  try {
    console.log('🚨 fetchBookingAlertsForUser called with user:', currentUser?.name, 'role:', currentUser?.role);
    
    if (!shouldFetchBookingAlerts(currentUser, hasRole)) {
      console.log('❌ Not fetching alerts - user is not staff');
      return [];
    }
    
    if (!currentUser?._id) {
      console.warn('No current user ID available for fetching booking alerts');
      return [];
    }
    ...
    if (!res.data || !res.data.data) {
      console.warn('Invalid booking alerts response format');
      return [];
    }
    ...
    const calendarAlerts = alerts.map(alert => {
      if (!alert._id || !alert.title || !alert.startTime || !alert.endTime) {
        console.warn('Invalid alert data structure:', alert);
        return null;
      }
    ...
    if (err.response?.status === 401) {
      console.error('Unauthorized access to booking alerts');
    } else if (err.response?.status === 403) {
      console.error('Forbidden access to booking alerts');
    } else if (err.response?.status >= 500) {
      console.error('Server error while fetching booking alerts');
    }
    return [];
  }
};
...
export const fetchLeaveRequestsForUser = async (currentUser, hasRole, startDateParam, endDateParam) => {
  try {
    console.log('🍃 fetchLeaveRequestsForUser called with user:', currentUser?.name, 'role:', currentUser?.role);
    
    if (!shouldFetchLeaveRequests(currentUser, hasRole)) {
      console.log('❌ Not fetching leave requests - no current user');
      return [];
    }
    
    if (!currentUser?._id) {
      console.warn('No current user ID available for fetching leave requests');
      return [];
    }
    ...
    if (err.response?.status === 401) {
      console.error('Unauthorized access to leave requests');
    } else if (err.response?.status === 403) {
      console.error('Forbidden access to leave requests');
    } else if (err.response?.status >= 500) {
      console.error('Server error while fetching leave requests');
    }
    return [];
  }
};
...
export const filterEventsByRole = (bookings, bookingAlerts, leaveRequests, currentUser, hasRole, selectedEmployees) => {
  ...
  console.log('🔍 Filtering events by role:', {
  ...
  if (hasRole(['staff'])) {
    ...
    const userBookings = bookings.filter(booking => {
      ...
      console.log('🔍 Staff booking check:', { staffId, userId, matches });
      return matches;
    });
    ...
    const staffLeaveRequests = leaveRequests.filter(leave => {
      ...
      console.log('🍃 Staff leave request check:', { leaveStaffId, userId, matches });
      return matches;
    });
    
    filtered = [...filtered, ...staffLeaveRequests];
    console.log('✅ Staff filtered events:', filtered.length);
    ...
    console.log('✅ Manager filtered events (selected):', filtered.length);
    ...
    console.log('✅ Manager filtered events (all):', filtered.length);
  }
