...
const TimesheetApprovalDashboard = () => {
  ...
  const fetchTimesheets = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('/timesheets/approval-status');
  ...
  const handleOverride = async () => {
    ...
    try {
      await axiosInstance.put(`/timesheets/${selectedTimesheet._id}/status`, {
        status: newStatus,
      ...
      setError('Failed to override timesheet status. Please try again.');
      console.error('Error overriding timesheet:', err);
    }
  };
